# Intel Arc Pro B60 – Einrichtung unter Ubuntu 26.04 (Resolute Raccoon)

Diese Anleitung beschreibt die vollständige Einrichtung einer Intel Arc Pro B60 GPU
(Battlemage / Xe2-Architektur) unter Ubuntu 26.04 LTS, inklusive OpenCL/Level-Zero-Treiber,
...
Ubuntu 26.04 LTS wird mit Kernel 6.14+ ausgeliefert – Xe2/Battlemage-Unterstützung ist damit
bereits im GA-Kernel enthalten. HWE ist nur bei sehr alten Installationen erforderlich.
...
Falls die Kernel-Version dennoch unter 6.8 liegt (z.B. bei Upgrade-Pfad von 24.04), den
HWE-Kernel installieren:
...
> **Hinweis zu Ubuntu 26.04:** Prüfe vorab auf der
> [Intel GPU Repository-Seite](https://dgpu-docs.intel.com/driver/client/overview.html),
> ob der `resolute-raccoon`-Stack bereits verfügbar ist. Falls Intel noch keinen dedizierten
> Stack für 26.04 veröffentlicht hat, kann übergangsweise der `noble`-Stack verwendet werden –
> die Pakete sind ABI-kompatibel.
...
**Option A – Resolute-Raccoon-Stack (sobald verfügbar):**

```bash
echo "deb [arch=amd64,i386 signed-by=/usr/share/keyrings/intel-graphics.gpg] \
  https://repositories.intel.com/gpu/ubuntu resolute-raccoon/lts/2350 unified" | \
  sudo tee /etc/apt/sources.list.d/intel-gpu-resolute-raccoon.list
```

**Option B – Noble-Stack als Fallback (ABI-kompatibel mit 26.04):**
...
> **Hinweis zu Ubuntu 26.04:** Prüfe zuerst, ob `nvtop` mit Intel-Unterstützung bereits in den
> offiziellen Ubuntu-Paketquellen enthalten ist:
> ```bash
> apt show nvtop
> ```
> Falls ja, genügt `sudo apt install nvtop` – der manuelle Build entfällt dann.
...
| Intel GPU-Repository nicht gefunden (404) | Noble-Stack als Fallback verwenden (siehe [Abschnitt 2.3](#23-intel-gpu-repository-unified-stack-hinzufügen)) |
